...
import UIKit
    ...
    final class LiveBTLEDebuggerTableViewController: UITableViewController, ContactEventRepositoryDelegate
    {
        ...
        func repository(_ repository: ContactEventRepository, didRecord _: IncomingBroadcastPayload, for _: BTLEPeripheral)
        {
            items = repository.contactEvents.sorted(by: { $0.timestamp < $1.timestamp })
        ...
        func repository(_: ContactEventRepository, didRecordRSSI _: Int, for _: BTLEPeripheral)
        {
            DispatchQueue.main.async
            {
                self.tableView.reloadData()
