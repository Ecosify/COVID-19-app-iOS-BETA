...
struct IncomingBroadcastPayload: Equatable, Codable
{
    ...
    init(data: Data)
    {
        countryCode = Int16(bigEndian: data.subdata(in: 0 ..< 2).to(type: Int16.self)!)
        cryptogram = data.subdata(in: 2 ..< 108)
        txPower = data.subdata(in: 108 ..< 109).to(type: Int8.self)!
        transmissionTime = Int32(bigEndian: data.subdata(in: 109 ..< 113).to(type: Int32.self)!)
        hmac = data.subdata(in: 113 ..< 129)
    }
}
