...
class ContactEventsUploader: ContactEventsUploading
{
    ...
    )
    {
        ...
        sessionDelegate = ContactEventsUploaderSessionDelegate(validator: trustValidator)
        session = makeSession(ContactEventsUploader.backgroundIdentifier, sessionDelegate)
    ...
    func cleanup()
    {
        let lastDate = persisting.uploadLog
            .compactMap
        { log in
            guard case let .started(date) = log.event else { return nil }
            return date
        }
